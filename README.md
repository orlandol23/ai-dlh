...
- **Replace-by-fee, same ordering as the initial send.** A transaction that does not confirm within the timeout is replaced on the same nonce with fees bumped 25 percent. The replacement is signed locally, its hash is journaled, and only then are the bytes handed to the node — never `sendTransaction`, which signs and broadcasts in one call and would learn the replacement's hash from an acknowledgement that can be lost. If the journal write reports the lock lost, the replacement is not broadcast at all.
- **Sign, journal, then broadcast; recover from receipts.** The path to a duplicate on-chain record is a row that has a transaction in flight and does not say so: the row stays `processing`, the stale lock expires, and the next worker sends a brand-new transaction for a completion that was already broadcast. For the initial send, signing, broadcasting and waiting are three separate calls, and the nonce and hash are written to the row **between the first two**. That ordering is possible because signing is local: a signed transaction already carries its final hash, so the row can name the exact transaction before any node has seen it. It matters because a broadcast whose acknowledgement is lost to a socket reset or an RPC timeout has already been accepted by the node — learning the hash from the acknowledgement leaves exactly the hole it was meant to close, with no crash required. The replace-by-fee path (see above) gives replacements the same guarantee: both the initial hash and every fee-bumped one are in the journal before their bytes go out. A reclaimed row is resolved from those receipts: every journaled hash is looked up, and the first one that mined decides the outcome — confirmed if status 1, permanently failed if it reverted — so a replacement that mined while its original was superseded confirms the record instead of reporting the nonce as consumed by an unknown transaction. When nothing mined, the worker re-broadcasts the SAME nonce instead of allocating a new one, so at most one transaction can ever take that slot. A nonce consumed by a transaction that is not in the journal stops the record instead of resending it: with every hash written down first, that means the custodial wallet was spent from elsewhere, which needs a human and not a retry.
...
What is guaranteed is **at most one on-chain record per completion**, and it is guaranteed by the nonce rather than by care. A completion never gets a second nonce: the first one is fixed before the transaction is signed and every later attempt recovers that same slot, so two transactions for one completion cannot both mine — the second is rejected as a nonce that is already used. The database backs this from the other side with a partial unique index on (user, module).

What is **not** guaranteed is that the completion always reaches the chain. Three outcomes park a record for a human instead: the attempt budget runs out, the contract reverts, or the journaled nonce turns out to have been consumed by a transaction this row never signed. All three are visible as `failed_permanent` with the error on the row, and none of them silently writes twice. Delivery is therefore best-effort with a bounded number of attempts; uniqueness is the part that is absolute.

The third case is worth separating from the other two, because it used to be much broader. While the hash was learned from the broadcast's acknowledgement, a lost acknowledgement whose transaction then mined landed here: the completion was on-chain, under a hash nobody had written down, and the row read `failed_permanent`. Journaling the signed hash before broadcasting removes that reading entirely, for the initial send and for fee-bump replacements alike — a transaction cannot exist without the row naming it — so what remains is the genuine case of the custodial wallet being spent from somewhere other than this queue.
...
266 tests pass across the three workspaces. Run each one directly:
